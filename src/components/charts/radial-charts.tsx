...
interface TemperatureRadialProps {
  value: number | null;
}

export const TemperatureRadial: React.FC<TemperatureRadialProps> = ({
  value,
}) => {
  const chartData = [{ temperature: value ?? 0, max: 100 }];
                  ...
                  return (
                    <text x={viewBox.cx} y={viewBox.cy} textAnchor="middle">
                      <tspan
                        x={viewBox.cx}
                        y={(viewBox.cy || 0) - 5}
                        className="fill-blue-600 dark:fill-blue-400 text-2xl font-bold"
                      >
                        {value !== null ? `${value}°C` : "--°C"}
                      </tspan>
                      <tspan
                        x={viewBox.cx}
                        y={(viewBox.cy || 0) + 20}
